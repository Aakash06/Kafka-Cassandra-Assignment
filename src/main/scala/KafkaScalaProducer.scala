...
object KafkaScalaProducer extends App {
  ...
  val obj = new TwitterFetcher
  val tweetsList = obj.searchUsingHashtags("jain_aakash06")

  tweetsList.onComplete {
    case Success(tweets) => {
      tweets.map {
        tweet =>
          val record: ProducerRecord[Nothing, String] = new ProducerRecord(topic, tweet)
          producer.send(record)
      }
    }
    case Failure(ex) => print("Sorry" + ex.getMessage)
  }
  /*
  for (i <- 1 to 50) {
    /**
